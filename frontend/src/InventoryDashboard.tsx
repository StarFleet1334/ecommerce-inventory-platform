...
const InventoryDashboard: React.FC = () => {
  ...
  const handleCreate = async (data: Record<string, any>) => {
    ...
    try {
      ...
      showNotification(
        `Failed to create ${activeTab.slice(0, -1)}. Please try again.`,
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setEntities((prev) => ({
        ...prev,
        [activeTab]: prev[activeTab as keyof typeof prev].filter(
          (item: any) => item[config.idField] !== id,
        ),
      }));
      ...
      <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <EntityList
            ...
            emptyMessage={`No ${activeTab} found. Add your first ${activeTab.slice(
              0,
              -1,
            )} above.`}
          />
